...
> Note: tab `value` strings must be unique — `glassTabsUI()` will error on
> duplicates. Values are plain strings; they do not need to match labels.
...
Use `compact = TRUE` when embedding glasstabs inside a tight layout such as a
`bs4Dash::bs4Card()`. It reduces margins, padding, and font size so the widget
...
`glassTabsUI()` works inside `bs4Dash::tabItem()`. Place `useGlassTabs()` in
the `dashboardBody()` (or anywhere before the first widget — once is enough).
...
> `useGlassTabs()` only needs to appear once. Inside a module that renders
> into a bs4Dash layout, call it in your module's UI function.
...
-   In a module, use `ns("tabs")` in the UI but pass the bare `"tabs"` to `glassTabsServer()` — not `ns("tabs")`. Passing a namespaced id (containing `-`) will produce a warning.
-   `glassTabPanel()` values must be unique within a `glassTabsUI()` call — duplicates cause an error.
