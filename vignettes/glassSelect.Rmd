...
It also supports server-side updates with `updateGlassSelect()`.
...
server <- function(input, output, session) {
  observe({
    message("Selected: ", input$pick %||% "NULL")
  })
}
```
...
`glassSelect()` does not require tabs. You only need `useGlassTabs()`
for the package assets:
